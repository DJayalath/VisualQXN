...
public class View extends Application {
    ...
    public void start(Stage stage) {
        ...
        CircuitController circuitController = new CircuitController(circuitPane);

        rootPane.setCenter(circuitPane);

        Scene scene = new Scene(rootPane);
        String css = (new File("art/stylesheet.css")).toURI().toString();

        stage.minWidthProperty().bind(circuitController.getCanvas().widthProperty());
...
//        Circuit circuit = new Circuit(4, 15);
//        circuit.draw();
...
//            addMeasure.setOnMouseClicked(e -> dialog.setResult(new QuantumMeter(circuit.getSelectedRow(), circuit.getSelectedCol())));
